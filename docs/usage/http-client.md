...
`loadSpec` | `Boolean=false`. This property will be present and set to `true` when the `Request` was constructed internally by `SwaggerClient` to fetch the OAS definition befined by `url` or when resolving remote JSON References.
