  ...
  if(this.securityDefinitions) {
    for(var key in this.securityDefinitions) {
      ...
      for(var ext in securityDefinition) {
        if (typeof ext === 'string' && ext.indexOf('x-') === 0) {
          securityDefinition.vendorExtensions[ext] = securityDefinition[ext];
          delete securityDefinition[ext];
        }
        if (ext === 'scopes') {
          ...
          if(typeof scopes === 'object') {
            ...
            for (var s in scopes) {
              if(typeof s === 'string' && s.indexOf('x-') === 0) {
                scopes.vendorExtensions[s] = scopes[s];
                delete scopes[s];
              }
            }
          }
    ...
    for (k = 0; k < response.tags.length; k++) {
      ...
      for(var i in t) {
        if(i === 'externalDocs' && typeof t[i] === 'object') {
          t[i].vendorExtensions = {};
          for(var j in t[i]) {
            if (typeof j === 'string' && j.indexOf('x-') === 0) {
              t[i].vendorExtensions[j] = t[i][j];
              delete t[i][j];
            }
          }
        }
        if(typeof i === 'string' && i.indexOf('x-') === 0) {
          t.vendorExtensions[i] = t[i];
          delete t[i];
        }
      }
    }
  ...
  _.forEach(response.paths, function (pathObj, path) {
    ...
    _.forEach(supportedOperationMethods, function (method) {
      ...
      for(var i in operation) {
        if(typeof i === 'string' && i.indexOf('x-') === 0) {
          operationObject.vendorExtensions[i] = operation[i];
        }
      }
      ...
      if(operationObject.externalDocs) {
        ...
        for(var i in operationObject.externalDocs) {
          if(typeof i === 'string' && i.indexOf('x-') === 0) {
            operationObject.externalDocs.vendorExtensions[i] = operationObject.externalDocs[i];
            delete operationObject.externalDocs[i];
          }
        }
      }
