  ...
  if(this.securityDefinitions) {
    for(var key in this.securityDefinitions) {
      ...
      for(var ext in securityDefinition) {
        extractExtensions(ext, securityDefinition);
        if (ext === 'scopes') {
          var scopes = securityDefinition[ext];
          if(typeof scopes === 'object') {
            scopes.vendorExtensions = {};
            for (var s in scopes) {
              extractExtensions(s, scopes);
            }
          }
        }
      }
    ...
    for (k = 0; k < response.tags.length; k++) {
      ...
      for(var i in t) {
        if(i === 'externalDocs' && typeof t[i] === 'object') {
          ...
          for(var j in t[i]) {
            extractExtensions(j, t[i]);
          }
        }
        extractExtensions(i, t);
      }
    }
  }
    ...
    _.forEach(supportedOperationMethods, function (method) {
      ...
      if(operationObject.externalDocs) {
        ...
        for(var i in operationObject.externalDocs) {
          extractExtensions(i, operationObject.externalDocs);
        }
      }
 ...
 * @param obj
 */
function extractExtensions(keyname, obj) {
  if(!keyname || !obj) {
    return;
  }
