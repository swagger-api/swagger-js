...
SwaggerClient.prototype.buildFromSpec = function (response) {
  ...
  if (Array.isArray(response.tags)) {
    ...
    for (k = 0; k < response.tags.length; k++) {
      ...
      t.order = k;  // remember the order of the tag
      ...
      _.forEach(tags, function (tag) {
        ...
        if (_.isUndefined(operationGroup)) {
          ...
          if (!_.isUndefined(tagDef)) {
            ...
            operationGroup.tagOrder = tagDef.order; // remember the tag order
          }

          self[clientProperty].help = _.bind(self.help, operationGroup);
          self.apisArray.push(new OperationGroup(tag, operationGroup.tagOrder, operationGroup.description, operationGroup.externalDocs, operationObject));
        }
