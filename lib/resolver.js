...
Resolver.prototype.resolve = function (spec, arg1, arg2, arg3) {
  ...
  for (name in spec.paths) {
    ...
    for (method in path) {
      ...
      if(method === '$ref') {
        ...
        for (i in parameters) {
          var parameter = parameters[i], location = '/paths' + name + '/' + method + '/parameters';
        ...
        for (responseCode in operation.responses) {
          ...
          var location = location = '/paths' + name + '/' + method + '/responses/' + responseCode;
  ...
  var resolved, opts = {}, expectedCalls = 0, toResolve = [];
...
Resolver.prototype.finish = function (spec, root, resolutionTable, resolvedRefs, unresolvedRefs, callback) {
  ...
  for (ref in resolutionTable) {
    var i, item = resolutionTable[ref];
    var key = item.key;
    ...
    if (resolvedTo) {
      ...
      if (item.resolveAs === 'ref') {
        ...
        for (key in resolvedTo.obj) {
          var abs = this.retainRoot(resolvedTo.obj[key]);
          targetObj[key] = abs;
        }
      }
    }
  ...
  if(this.iteration === 2) {
    // console.log('ref count: ' + this.countRefs(spec) + ', unresolvedRefs: ' + Object.keys(unresolvedRefs).length);
    callback.call(this.scope, spec, unresolvedRefs);
  }
  else {
    this.iteration += 1;
...
Resolver.prototype.resolveInline = function (root, spec, property, resolutionTable, unresolvedRefs, location) {
  ...
  if (ref) {
    ...
    if (ref.indexOf('http') === 0) {
      ...
      resolutionTable.push({obj: property, resolveAs: 'inline', root: root, key: ref, location: location});
