...
SwaggerSpecConverter.prototype.declaration = function(obj, swagger) {
  ...
  for(i = 0; i < obj.apis.length; i++) {
    ...
    this.operations(path, obj.resourcePath, operations, swagger);
  }
...
SwaggerSpecConverter.prototype.extractTag = function(resourcePath) {
  return (resourcePath || 'default').replace('/', '');
}

SwaggerSpecConverter.prototype.operations = function(path, resourcePath, obj, swagger) {
  ...
  for(i = 0; i < obj.length; i++) {
    ...
    if(existingOperation.authorizations) {
      for(var key in existingOperation.authorizations) {
        operation.security = operation.security || [];
        var scopes = existingOperation.authorizations[key];
        if(scopes) {
          var securityScopes = [];
          for(var j in scopes) {
