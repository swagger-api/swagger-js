...
var schemaToHTML = function (name, schema, models) {
  ...
  var primitiveToOptionsHTML = function (schema) {
    var html = '';
    var type = schema.type || 'object';
    ...
    if (schema.default) {
      html += helpers.optionHtml('Default', schema.default);
    }

    switch (type) {
    case 'string':
      if (schema.minLength) {
        html += helpers.optionHtml('Min. Length', schema.minLength);
      }

      if (schema.maxLength) {
        html += helpers.optionHtml('Max. Length', schema.maxLength);
      }

      if (schema.pattern) {
        html += helpers.optionHtml('Reg. Exp.', schema.pattern);
      }
      break;
    case 'integer':
    case 'number':
      if (schema.minimum) {
        html += helpers.optionHtml('Min. Value', schema.minimum);
      }

      if (schema.exclusiveMinimum) {
        html += helpers.optionHtml('Exclusive Min.', 'true');
      }

      if (schema.maximum) {
        html += helpers.optionHtml('Max. Value', schema.maximum);
      }

      if (schema.exclusiveMaximum) {
        html += helpers.optionHtml('Exclusive Max.', 'true');
      }

      if (schema.multipleOf) {
        html += helpers.optionHtml('Multiple Of', schema.multipleOf);
      }
    ...
    if (isArray) {
      if (schema.minItems) {
        html += helpers.optionHtml('Min. Items', schema.minItems);
      }

      if (schema.maxItems) {
        html += helpers.optionHtml('Max. Items', schema.maxItems);
      }

      if (schema.uniqueItems) {
        html += helpers.optionHtml('Unique Items', 'true');
      }

      if (schema.collectionFormat) {
        html += helpers.optionHtml('Coll. Format', schema.collectionFormat);
      }
    }
    ...
    if (_.isArray(schema.enum)) {
      ...
      html += helpers.optionHtml('Enum', enumString);
    }
      ...
      if (schema.$ref) {
        ...
        if (_.isPlainObject(schema.properties)) {
          html += _.map(schema.properties, function (property, name) {
            ...
            options = primitiveToOptionsHTML(property);

            if (options.length > 0) {
              html = '<span class="propWrap">' + html + '<table class="optionsWrapper"><tr><th colspan="2">' + name + '</th></tr>' + options + '</table></span>';
            }
        ...
        options = primitiveToOptionsHTML(schema);
