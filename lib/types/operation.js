...
Operation.prototype.getBody = function (headers, args, opts) {
  ...
  // handle form params
  // JSON encode formData if we are consuming application/json
  if (hasFormParams && headers['Content-Type'] == 'application/json'){
    var prams = {};
    for (var i = 0; i < this.parameters.length; i++) {
      var param = this.parameters[i];
      if (typeof args[param.name] !== 'undefined') {
        prams[param.name] = args[param.name];
      }
    }
    body = prams;
  }else if (hasFormParams && !isMultiPart) {
    var encoded = '';
