...
Operation.prototype.urlify = function (args) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if (param.in === 'path') {
      var paramTemplate = requestUrl.match(new RegExp('\{(/?)' + param.name + '(\\*?)\}', 'i'));
      var value = (typeof args[param.name] !== 'undefined') ? args[param.name] : '';

      if (Array.isArray(value)) {
        value = this.encodePathCollection(param.collectionFormat, param.name, value);
      } else {
        if (paramTemplate[2] === '*') {
          value = this.encodePathParamWithoutSlash(value);
        } else {
      ...
      if (value !== '') {
        value = paramTemplate[1] + value;
      }
      requestUrl = requestUrl.replace(paramTemplate[0], value);
    ...
    } else if (param.in === 'formData' && typeof args[param.name] !== 'undefined') {
...
Operation.prototype.encodePathParamWithoutSlash = function (pathParam) {
  var splited = pathParam.split('/');
