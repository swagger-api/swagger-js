...
var Operation = module.exports = function (parent, scheme, operationId, httpMethod, path, args, definitions, models, clientAuthorizations) {
  ...
  if (response && response.schema) {
    ...
    } else if (!response.schema.type || response.schema.type === 'object') {
      // Inline model
      this.successResponse = {};
      successResponse = this.successResponse[defaultResponseCode] = new Model('Inline Response (' + this.method + ':' + this.path + ')', response.schema || {}, this.models);
    } else {
      // Primitive
      this.successResponse = {};
