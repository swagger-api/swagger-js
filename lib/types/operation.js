...
var OperationParams = require( '../types/OperationParams' );
var SwaggerHttp = require('../http');

function convertArgs( args, operation ){
  if( !(args && args instanceof OperationParams) ){
    args = args || {};
    args.parameters = operation.parameters;
    args = new OperationParams( args );
  }
  return args;
}
...
Operation.prototype.getHeaderParams = function (args) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if ( param.in === 'header' && args.exists( param.in, param.name ) ) {
        var value = args.get( param.in, param.name );
...
Operation.prototype.urlify = function (args) {
  ...
  args = convertArgs( args, this );

  for (var i = 0; i < this.parameters.length; i++) {
    var param = this.parameters[i];

        if (param.in === 'path' && args.exists( param.in, param.name )) {
        ...
        } else if (param.in === 'query' && args.exists( param.in, param.name )) {
          if (querystring === '') {
            querystring += '?';
          } else {
            querystring += '&';
          ...
          if (typeof param.collectionFormat !== 'undefined') {
            var qp = args.get( param.in, param.name);

            if (Array.isArray(qp)) {
              querystring += this.encodeQueryCollection(param.collectionFormat, param.name, qp);
            } else {
              querystring += this.encodeQueryParam(param.name) + '=' +
              this.encodeQueryParam(qp);
            }
          } else {
            querystring += this.encodeQueryParam(param.name) + '=' +
             this.encodeQueryParam(args.get(param.in,param.name));
          }
        } else if (param.in === 'formData' && args.exists( param.in, param.name )) {
          formParams[param.name] = args.get( param.in, param.name );
        }
  }
...
Operation.prototype.getMissingParams = function (args) {
  ...
  for (i = 0; i < this.parameters.length; i++) {
    var param = this.parameters[i];
    if (param.required === true && !args.exists( param.in, param.name ) ) {
...
Operation.prototype.getBody = function (headers, args, opts) {
  ...
  for (var i = 0; i < this.parameters.length; i++) {
    ...
    if ( args.exists( param.in, param.name ) ) {
      if (param.in === 'body') {
        body = args.get( param.in, param.name );
...
Operation.prototype.execute = function (arg1, arg2, arg3, arg4, parent) {
  var args = convertArgs( arg1, this );
...
Operation.prototype.setContentTypes = function (args, opts) {
  ...
  for (i = 0; i < allDefinedParams.length; i++) {
    ...
    } else if (param.in === 'body' && args.exists( param.in, param.name ) ) {
      body = args.get( param.in, param.name );
    }
  }
