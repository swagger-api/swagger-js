...
Resolver.prototype.resolve = function(spec, callback, scope) {
  ...
  // models
  _.each(spec.definitions, function(model){
    if (!model.properties) {
      model.properties = {};
    }
    if (model.allOf) {
      _.each(model.allOf, function(merge){
        if(merge.$ref){
          model.$ref = merge.$ref;
          this.resolveInline(spec, model, resolutionTable, unresolvedRefs);
        }else{
          _.each(merge.properties, function(property, propertyName){
  ...
  _.each(spec.paths, function(path){
    _.each(path, function(operation){
      ...
      _.each(operation.responses, function(response){
        if(response.schema) {
  ...
  _.each(resolutionTable, function(element, name){
...
Resolver.prototype.finish = function(spec, resolutionTable, resolvedRefs, unresolvedRefs, callback) {
  ...
  var ref;
  _.each(resolutionTable, function(locations, ref){
    for(var i = 0; i < locations.length; i++) {
      ...
      if(resolvedTo) {
        ...
        if(locations[i].resolveAs === '$ref') {
          ...
          _.each(resolvedTo.obj, function(element, key){
...
Resolver.prototype.resolveInline = function (spec, property, objs, unresolvedRefs) {
  ...
  if(ref) {
    if(ref.indexOf('http') === 0) {
      ...
      if(location) {
        delete property.$ref;
        _.each(location, function(element, key){
