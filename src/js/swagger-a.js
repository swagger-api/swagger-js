 ...
 */
SwaggerClient.prototype.resolve = function(spec, callback) {
  ...
  for(name in spec.definitions) {
    ...
    for(propertyName in model.properties) {
      ...
      this.resolveProperty(property, objs);
    }
  }
  // operations
  for(name in spec.paths) {
    var operation;
    var path = spec.paths[name];
    for(method in path){
      ...
      for(i in parameters) {
        ...
        if(parameter.in === 'body') {
          this.resolveProperty(parameter, objs);
        }
      }
      var responseCode;
      for(responseCode in operation.responses) {
        ...
        if(response.schema) {
          this.resolveProperty(response.schema, objs);
        }
      }
    }
  }
...
SwaggerClient.prototype.resolveProperty = function (property, objs) {
  ...
  if(ref) {
    ...
    this.resolveProperty(items, objs);
  }
}
